...
function purchase() {
	...
	this. Listpo = function (req, res) {
		connection.acquire(function (err, con) {
			con.query('SELECT a.`id`,a.`createddate`,a.`vendor`,(SELECT `name` FROM `vendor` WHERE `id` = a.vendor) as vendorname,(SELECT `name` FROM `products` WHERE `id` = b.productid)as productname,a.`netamount` FROM `pomaster` as a, `podetails` as b where a.id=b.poid  ORDER BY id desc', function (err, result) {
				con.release();
				res.send(result);
			});
		});
	...
	this.SaveGrDetails= function (grdetails, res) {
		console.log(grdetails)
		 connection.acquire(function (err, con) {
			con.query("INSERT INTO `grmaster`(`poid`, `invoiceno`, `invoicedate`,`grossamount`, `taxamount`, `netamount`, `cgst`, `sgst`, `igst`, `paymentstatus`, `createdby`) VALUES (?,?,?,?,?,?,?,?,?,?,?)",[grdetails[0].masterid,grdetails[0].invoiceno,grdetails[0].invoicedate,grdetails[0].grossamount,grdetails[0].taxamount,grdetails[0].netamount,grdetails[0].cgst,grdetails[0].sgst,grdetails[0].igst,0,grdetails[0].createdby], function (err, result) {
				if(err)
				{
				...
				else
				{
					...
					var ss = '';
					grdetails.forEach((item) =>{
							ss+= '('+insertid+','+item.productid+','+item.porate+','+item.qty+','+item.netvalue+'),'
					});
					
					ss = ss.substr(0, ss.length - 1);
					
					con.query("INSERT INTO `grdetails`(`grid`, `productsid`, `porate`, `qty`,`netvalue`) VALUES "+ss, function (err, result) {
				if(err)
				{
					con.query("DELETE FROM `grmaster` WHERE `id` = "+insertid, function (err, result) {
						if(err)
						{
	...
	this.Addvendorpayment= function (user, res) {
		connection.acquire(function (err, con) {
				con.query('INSERT INTO vendorpayment(poid,vendor,amount,grinvoiceno, grinvoicedate, chqno,chqdate, bank,paidamount,paymentmode,pendingamount,transactionid,transactiondate,createdby,recievedby) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',[user.poid,user.vendor,user.netamount,user.invoiceno,user.invoicedate,user.chqno,user.chqdate,user.bank,user.paidamount,user.paymentmode,user.pendingamount,user.transactionid,user.transactiondate,user.createdby,user.recievedby], function (err, result) {
					con.release();
					if(err)
								{res.send({status:1,message:"Failed To Insert Data"});}
								else
	...
	this. ListvendorsPaymets = function (req, res) {
		connection.acquire(function (err, con) {
			...
			con.query('select *,(select name from vendor where id=vendorpayment.vendor)as vendorname,(vendorpayment.amount - (SELECT SUM(`paidamount`) FROM vendorpayment a WHERE a.poid = vendorpayment.poid AND a.vendor =vendorpayment.vendor)) as totalpendingamt,(SELECT SUM(b.paidamount) FROM vendorpayment b WHERE b.poid = vendorpayment.poid) as totalpaidamt from vendorpayment GROUP BY vendorpayment.poid order by id desc', function (err, result) {
				con.release();
			res.send(result);
			});
		});
	...
	this. getPaymentDetails = function (id, res) {
	 ...
	 this.EditpaymentDetails = function(user, res) {
		...
		connection.acquire(function(err, con) {
		 ...
		 con.query('INSERT INTO vendorpayment(poid,vendor,amount,grinvoiceno, grinvoicedate,paidamount,paymentmode,chqno,chqdate, bank,pendingamount,transactionid,transactiondate,createdby,recievedby) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',[user[0].poid,user[0].vendor,user[0].amount,user[0].grinvoiceno,user[0].grinvoicedatestr,user[0].paidamountnew,user[0].paymentmodenew,user[0].chqnonew,user[0].chqdatenew,user[0].banknew,user[0].pendingamountnew,user[0].transactionid,user[0].transactiondate,user[0].username,user[0].recievedby], function(err1, result1) {
        con.release();
		console.log(err1)
        if (err1) {
  ...
  this.Deletevendorpayment= function (id, res) {
	...
	this.listGrForStockEntry= function (req, res) {
		connection.acquire(function (err, con) {
			con.query('SELECT `id`,poid FROM `grmaster` WHERE poid NOT IN (SELECT `grid` FROM stocksrn WHERE stocksrn.productsid IN (SELECT grdetails.productsid FROM grdetails WHERE grdetails.grid IN (SELECT grmaster.id FROM grmaster)))', function (err, result) {
				con.release();
				if(err)
					{
						res.send({status:1,message:"NO DATA FOUND"});
	...
	this.ListPurchaeStock= function (req, res) {
		connection.acquire(function (err, con) {
			/* con.query('SELECT (SELECT products.productstype FROM products WHERE products.id = grdetails.productsid) as producttype,grdetails.productsid,grdetails.qty,(SELECT products.name FROM products WHERE products.id = grdetails.productsid) as productname,grmaster.poid,grmaster.invoiceno,grmaster.invoicedate,(grdetails.qty - (SELECT COUNT(*) FROM stocksrn WHERE stocksrn.grid = grmaster.id AND stocksrn.productsid = grdetails.productsid)) as qtyavl FROM grmaster INNER JOIN grdetails on grdetails.grid = grmaster.id ORDER BY grmaster.id DESC', function (err, result) { */
				
				con.query('SELECT grmaster.id as grid,(SELECT products.productstype FROM products WHERE products.id = grdetails.productsid) as producttype,(CASE WHEN (SELECT products.productstype FROM products WHERE products.id = grdetails.productsid) = "Qty.-Meters" THEN (grdetails.qty * grdetails.measure) ELSE grdetails.qty END) as actqty,grdetails.productsid,grdetails.qty,(SELECT products.name FROM products WHERE products.id = grdetails.productsid) as productname,grdetails.measure,grmaster.poid,grmaster.invoiceno,grmaster.invoicedate,(CASE WHEN (SELECT products.productstype FROM products WHERE products.id = grdetails.productsid) = "Qty.-Meters" THEN ((grdetails.qty * grdetails.measure) - (IFNULL((SELECT SUM(stocksrn.endmeter) FROM stocksrn WHERE stocksrn.poid = grmaster.poid AND stocksrn.productsid = grdetails.productsid),0) - IFNULL((SELECT SUM(stocksrn.startmeter) FROM stocksrn WHERE stocksrn.poid = grmaster.poid AND stocksrn.productsid = grdetails.productsid),0))) ELSE (grdetails.qty - (SELECT COUNT(*) FROM stocksrn WHERE stocksrn.grid = grmaster.id AND stocksrn.productsid = grdetails.productsid)) END) as qtyavl FROM grmaster INNER JOIN grdetails on grdetails.grid = grmaster.id ORDER BY grmaster.id DESC', function (err, result) {
				con.release();
				console.log(err);
				if(err)
					{
	...
	this.getUsedItems= function (itemdetails, res) {
		console.log(itemdetails);
		 connection.acquire(function (err, con) {
			con.query('SELECT `id`,`poid`,`productsid`, `srno`, `startmeter`, `endmeter`, `assignemp`, `address`, `zone`,`assigndate`, `assignedfrom` FROM `stocksrn` WHERE `productsid` = '+itemdetails.productsid+' AND `poid` = '+itemdetails.poid, function (err, result) {
				console.log(err);
				if(err)
					{
						res.send({status:1,message:"NO DATA FOUND"});
					...
					else
					{
						...
						if(itemdetails.producttype === 'Qty.-Meters')
						{
							if(result.length > 0)
						{
							var avlqty = result.reduce((sum,value)=>{
								return sum + (parseFloat(value.endmeter) - parseFloat(value.startmeter));
							},0)
							if(avlqty < itemdetails.actqty)
							{
								result.push({})
							}
							if(avlqty == itemdetails.actqty)
					...
					else
					{
						...
						if(0 < result.length == itemdetails.actqty)
						{
							result = result;
						}
						if(result.length < itemdetails.actqty)
						{
							for(var i = 0 ; i < (itemdetails.actqty - result.length);i++)
							{
								result.push({})
							}
	...
	this.SaveSRNData= function (srndata, res) {
		
		console.log(srndata);
		if(srndata.id)
		{
			var sql = 'UPDATE `stocksrn` SET ? WHERE id = ?';
			var dataobj = [srndata,srndata.id];
		}
		else
		{
			srndata.assigndate = new Date(srndata.assigndate);
			var sql = 'INSERT INTO `stocksrn` SET ?'
			var dataobj = srndata;
		}
		 connection.acquire(function (err, con) {
			con.query(sql,dataobj, function (err, result) {
				...
				if(err)
					...
					{
						res.send({status:0,message:'record saved successfully.'});
					}
			});
		});  
	};
