...
function user() {
    ...
    this.userListBirthday = function (req, res) {
        connection.acquire(function (err, con) {
            con.query('SELECT name as fullname FROM `employeemaster` WHERE DATE_FORMAT(employeemaster.dob,"%M-%D") = DATE_FORMAT(CURDATE(),"%M-%D")', function (err, result) {
                con.release();
                if (err) {
                    res.send('No data Found');
                } else {
	...
	this.getSupervisordetails = function (userlevel,res) {
        connection.acquire(function (err, con) {
			...
			{
				var sql = 'SELECT `id`,`username` FROM `user` WHERE `userlevel` = "HO" OR `userlevel` = "SUPERVISOR" OR `userlevel` = "FIELD" OR `userlevel` = "OFFICE" ';
			}
	...
	this.userAttendance = function (user, res) {
		...
		var url =  "https://reverse.geocoder.api.here.com/6.2/reversegeocode.json?prox=" + user.lat + ","+ user.lan + "&mode=retrieveAddresses&maxresults=1&gen=9&app_id=F2DrM7GiRxDODD2SR10H&app_code=KA7GSs5492GnQ5Xxzq791Q"
					...
					https.get(url, (resp) => {
						  ...
						  resp.on('end', () => {
							  ...
							  var addressobj = JSON.parse(data).Response.View[0].Result[0].Location.Address
								...
								var formatedaddress = addressobj.Label+'- '+addressobj.PostalCode+'.';
	...
	var sql = con1.query('SELECT * FROM `userattendance` WHERE DATE_FORMAT(attdate,"%Y-%m-%d") = CURDATE() AND `userid` ='+user.userid, function (err, result1) {
			con1.release();
                if (result1!=undefined && result1.length == 0) {
					...
					connection.acquire(function (err, con) {
						con.query('INSERT INTO userattendance (`userid`,`attdate`,`intime`,`inaddress`, `status`) SELECT * FROM (SELECT '+user.userid+',CURDATE(),NOW(),"'+formatedaddress+'",1) AS tmp WHERE NOT EXISTS (SELECT userid FROM userattendance WHERE attdate = CURDATE() AND userid = '+user.userid+')', function (err, result) {
							con.release();
							console.log(err);
							if (err) {
								res.send({
						...
						connection.acquire(function (err, con) {
							...
							con.query('SELECT * FROM `userattendance` WHERE DATE_FORMAT(`intime`,"%D-%M-%y") = DATE_FORMAT(CURDATE(),"%D-%M-%y") AND `intime` > (NOW() - INTERVAL 2 HOUR) AND userid = '+user.userid, function(err, intimeResult){
									if(intimeResult.length > 0)
									{
										res.send({
											status: 3,
											message: 'Please try after two hours.'
										});
									}
									else
									{
										con.query('UPDATE userattendance set `outaddress` = "'+formatedaddress+'", outtime	 = now(),`status` = 0 where `userid` ='+user.userid+' and DATE_FORMAT(attdate,"%Y-%m-%d") = CURDATE()', function (err, result) {
											con.release();
											if (err) {
												res.send({
													status: 3,
							...
							var sql = con.query('SELECT * FROM `userattendance` WHERE DATE_FORMAT(attdate,"%Y-%m-%d") = CURDATE() AND `userid` ='+ result[0].id, function (err, result1) {
								...
								console.log('Authenticated.');
												...
												res.json({
														...
														sipid: result['0'].sipid,
														sidpassword: result['0'].sidpassword,
														siplink: result[0].sipip,
														token: token
												});
  ...
  /*----monisha code--------*/
   ...
   this.allorderreport = function(res) {
    ...
    connection.acquire(function(err, con) {
			...
			{
			var sql= 'select a.*,b.*,a.id as purchaseid,DATE_FORMAT(a.createddate,"%d %b %Y")as doe,(select name from products where b.productid=products.id)as productname,(select name from vendor where id=a.vendor)as vendorname,(select vendor.name from vendor where id=a.vendor)as vendorsrchname,IFNULL((SELECT SUM(vendorpayment.paidamount) FROM vendorpayment WHERE vendorpayment.vendor = a.vendor AND vendorpayment.poid = a.id),0) as totalpaidamt from pomaster a,podetails b where a.id=b.poid and DATE_FORMAT(a.createddate,"%Y-%m-%d") = "'+fromdate+'" and a.vendor = '+vendorid+' order by a.id desc';
			}
			
			if(todate == "undefined" && vendorid == 'undefined')
			{
			var sql= 'select a.*,b.*,a.id as purchaseid,DATE_FORMAT(a.createddate,"%d %b %Y")as doe,(select name from products where b.productid=products.id)as productname,(select name from vendor where id=a.vendor)as vendorname,(select vendor.name from vendor where id=a.vendor)as vendorsrchname,IFNULL((SELECT SUM(vendorpayment.paidamount) FROM vendorpayment WHERE vendorpayment.vendor = a.vendor AND vendorpayment.poid = a.id),0) as totalpaidamt from pomaster a,podetails b where a.id=b.poid and DATE_FORMAT(a.createddate,"%Y-%m-%d") = "'+fromdate+'" order by a.id desc';
			}
		
		
			if(todate != 'undefined' && vendorid != 'undefined')
			{
				var sql= 'select a.*,b.*,a.id as purchaseid,DATE_FORMAT(a.createddate,"%d %b %Y")as doe,(select name from products where b.productid=products.id)as productname,(select name from vendor where id=a.vendor)as vendorname,(select vendor.name from vendor where id=a.vendor)as vendorsrchname,IFNULL((SELECT SUM(vendorpayment.paidamount) FROM vendorpayment WHERE vendorpayment.vendor = a.vendor AND vendorpayment.poid = a.id),0) as totalpaidamt from pomaster a,podetails b where a.id=b.poid and and a.vendor = '+vendorid+' and (DATE_FORMAT(a.createddate,"%Y-%m-%d") BETWEEN  "'+fromdate+'"  AND "'+todate+'" ) order by a.id desc';
			}
			if(todate != 'undefined' &&  vendorid == 'undefined')
			{
				var sql= 'select a.*,b.*,a.id as purchaseid,DATE_FORMAT(a.createddate,"%d %b %Y")as doe,(select name from products where b.productid=products.id)as productname,(select name from vendor where id=a.vendor)as vendorname,(select vendor.name from vendor where id=a.vendor)as vendorsrchname,IFNULL((SELECT SUM(vendorpayment.paidamount) FROM vendorpayment WHERE vendorpayment.vendor = a.vendor AND vendorpayment.poid = a.id),0) as totalpaidamt from pomaster a,podetails b where a.id=b.poid and (DATE_FORMAT(a.createddate,"%Y-%m-%d") BETWEEN  "'+fromdate+'"  AND "'+todate+'") order by a.id desc';
			}
...
this.allvendorreports = function(res) {
	  connection.acquire(function(err, con) {
		  ...
		  var sql = 'SELECT v.id,v.poid,v.grinvoicedate,v.vendor,v.createddate,(SELECT pomaster.createddate FROM pomaster WHERE pomaster.id = v.poid) as podate,(SELECT vendor.name FROM vendor WHERE vendor.id = v.vendor) as vendorname,v.paidamount,v.amount,(SELECT SUM(vendorpayment.paidamount) FROM vendorpayment WHERE vendorpayment.vendor = v.vendor) as totalpaidamount,(SELECT SUM(vendorpayment.amount) FROM vendorpayment WHERE vendorpayment.vendor = v.vendor) as totalamount,(IFNULL((SELECT SUM(vendorpayment.amount) FROM vendorpayment WHERE vendorpayment.vendor = v.vendor),0) - IFNULL((SELECT SUM(vendorpayment.paidamount) FROM vendorpayment WHERE vendorpayment.vendor = v.vendor),0)) AS pendingamt,(SELECT SUM(podetails.qty) FROM podetails WHERE podetails.poid = v.poid) as totalqty,(SELECT GROUP_CONCAT((SELECT products.name FROM products WHERE products.id = podetails.productid)) FROM podetails WHERE podetails.poid = v.poid) as productslist FROM vendorpayment v';
		  //'select a.*,b.*,a.id as purchaseid,DATE_FORMAT(a.createddate,"%d %b %Y")as doe,(select name from products where b.productid=products.id)as productname,(select name from vendor where id=a.vendor)as vendorname,(select vendor.name from vendor where id=a.vendor)as vendorsrchname,IFNULL((SELECT SUM(vendorpayment.paidamount) FROM vendorpayment WHERE vendorpayment.vendor = a.vendor AND vendorpayment.poid = a.id),0) as totalpaidamt from pomaster a,podetails b where a.id=b.poid order by a.id desc'
		con.query(sql, function(err, result) {
	   con.release();
        res.send(result);
      });
...
this.GetVendorviseReport = function(vendorid,fromdate,todate,res) {
	  connection.acquire(function(err, con) {
		  ...
		  if(todate == "undefined" && fromdate == "undefined")
			{
			//var sql= 'select a.*,b.*,a.id as purchaseid,DATE_FORMAT(a.createddate,"%d %b %Y")as doe,(select name from products where b.productid=products.id)as productname,(select name from vendor where id=a.vendor)as vendorname,(select vendor.name from vendor where id=a.vendor)as vendorsrchname,IFNULL((SELECT SUM(vendorpayment.paidamount) FROM vendorpayment WHERE vendorpayment.vendor = a.vendor AND vendorpayment.poid = a.id),0) as totalpaidamt from pomaster a,podetails b where a.id=b.poid and a.vendor = '+vendorid+' order by a.id desc'
			var sql= 'SELECT v.id,v.poid,v.grinvoicedate,v.vendor,v.createddate,(SELECT pomaster.createddate FROM pomaster WHERE pomaster.id = v.poid) as podate,(SELECT vendor.name FROM vendor WHERE vendor.id = v.vendor) as vendorname,v.paidamount,v.amount,(SELECT SUM(vendorpayment.paidamount) FROM vendorpayment WHERE vendorpayment.vendor = v.vendor) as totalpaidamount,(SELECT SUM(vendorpayment.amount) FROM vendorpayment WHERE vendorpayment.vendor = v.vendor) as totalamount,(IFNULL((SELECT SUM(vendorpayment.amount) FROM vendorpayment WHERE vendorpayment.vendor = v.vendor),0) - IFNULL((SELECT SUM(vendorpayment.paidamount) FROM vendorpayment WHERE vendorpayment.vendor = v.vendor),0)) AS pendingamt,(SELECT SUM(podetails.qty) FROM podetails WHERE podetails.poid = v.poid) as totalqty,(SELECT GROUP_CONCAT((SELECT products.name FROM products WHERE products.id = podetails.productid)) FROM podetails WHERE podetails.poid = v.poid) as productslist FROM vendorpayment v WHERE v.vendor = '+vendorid
			...
			if(todate == "undefined" && fromdate != 'undefined')
			{
				...
				var sql= 'SELECT v.id,v.poid,v.grinvoicedate,v.vendor,v.createddate,(SELECT pomaster.createddate FROM pomaster WHERE pomaster.id = v.poid) as podate,(SELECT vendor.name FROM vendor WHERE vendor.id = v.vendor) as vendorname,v.paidamount,v.amount,(SELECT SUM(vendorpayment.paidamount) FROM vendorpayment WHERE vendorpayment.vendor = v.vendor) as totalpaidamount,(SELECT SUM(vendorpayment.amount) FROM vendorpayment WHERE vendorpayment.vendor = v.vendor) as totalamount,(IFNULL((SELECT SUM(vendorpayment.amount) FROM vendorpayment WHERE vendorpayment.vendor = v.vendor),0) - IFNULL((SELECT SUM(vendorpayment.paidamount) FROM vendorpayment WHERE vendorpayment.vendor = v.vendor),0)) AS pendingamt,(SELECT SUM(podetails.qty) FROM podetails WHERE podetails.poid = v.poid) as totalqty,(SELECT GROUP_CONCAT((SELECT products.name FROM products WHERE products.id = podetails.productid)) FROM podetails WHERE podetails.poid = v.poid) as productslist FROM vendorpayment v WHERE v.vendor = '+vendorid+' AND DATE_FORMAT(v.createddate,"%Y-%m-%d") = "'+fromdate+'"';
			...
			if(todate != 'undefined' && fromdate != 'undefined')
			{
				//var sql= 'select a.*,b.*,a.id as purchaseid,DATE_FORMAT(a.createddate,"%d %b %Y")as doe,(select name from products where b.productid=products.id)as productname,(select name from vendor where id=a.vendor)as vendorname,(select vendor.name from vendor where id=a.vendor)as vendorsrchname,IFNULL((SELECT SUM(vendorpayment.paidamount) FROM vendorpayment WHERE vendorpayment.vendor = a.vendor AND vendorpayment.poid = a.id),0) as totalpaidamt from pomaster a,podetails b where a.id=b.poid  and a.vendor = '+vendorid+' and (DATE_FORMAT(a.createddate,"%Y-%m-%d") BETWEEN  "'+fromdate+'"  AND "'+todate+'" ) order by a.id desc';
				
				var sql= 'SELECT v.id,v.poid,v.grinvoicedate,v.vendor,v.createddate,(SELECT pomaster.createddate FROM pomaster WHERE pomaster.id = v.poid) as podate,(SELECT vendor.name FROM vendor WHERE vendor.id = v.vendor) as vendorname,v.paidamount,v.amount,(SELECT SUM(vendorpayment.paidamount) FROM vendorpayment WHERE vendorpayment.vendor = v.vendor) as totalpaidamount,(SELECT SUM(vendorpayment.amount) FROM vendorpayment WHERE vendorpayment.vendor = v.vendor) as totalamount,(IFNULL((SELECT SUM(vendorpayment.amount) FROM vendorpayment WHERE vendorpayment.vendor = v.vendor),0) - IFNULL((SELECT SUM(vendorpayment.paidamount) FROM vendorpayment WHERE vendorpayment.vendor = v.vendor),0)) AS pendingamt,(SELECT SUM(podetails.qty) FROM podetails WHERE podetails.poid = v.poid) as totalqty,(SELECT GROUP_CONCAT((SELECT products.name FROM products WHERE products.id = podetails.productid)) FROM podetails WHERE podetails.poid = v.poid) as productslist FROM vendorpayment v WHERE  (DATE_FORMAT(v.createddate,"%Y-%m-%d") BETWEEN  "'+fromdate+'"  AND "'+todate+'" ) AND v.vendor = '+vendorid;
			}
 ...
 this.salereturndatereport = function(fromdate,todate,res) {
	 ...
	 this.SetTokenForPush = function(regdetails,res) {
